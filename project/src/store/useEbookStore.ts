...
export const useEbookStore = create<EbookStore>((set, get) => ({
  ...
  addChapter: (chapter) =>
    set((state) => {
      // Sort chapters by type to maintain order: frontmatter -> chapter -> backmatter
      const newChapter = {
      ...
      const sortedChapters = [...state.chapters, newChapter].sort((a, b) => {
        const typeOrder = { frontmatter: 0, chapter: 1, backmatter: 2 };
  ...
  updateChapter: (id, chapter) =>
    set((state) => {
      ...
      // Re-sort chapters to maintain order
  ...
  reorderChapters: (chapters) => {
    // Ensure chapters are sorted by type even when manually reordered
    const sortedChapters = chapters.sort((a, b) => {
      const typeOrder = { frontmatter: 0, chapter: 1, backmatter: 2 };
      return typeOrder[a.type] - typeOrder[b.type];
    });
    
    set({ chapters: sortedChapters });
  ...
  calculatePageNumbers: () => {
    ...
    const updatedChapters = chapters.map((chapter) => {
      ...
      // Add pages for chapter content and spacing
      const totalPages = Math.max(1, contentPages + imagePages);
