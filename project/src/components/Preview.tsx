...
export function Preview() {
  ...
  const generatePdf = async () => {
    try {
      ...
      doc.text(settings.title || 'Untitled', pageWidth / 2, currentY + 40, { align: 'center' });
      
      doc.setFont(settings.fonts.subtitle.family);
      doc.setFontSize(settings.fonts.subtitle.size);
      doc.text(settings.author || '', pageWidth / 2, currentY + 60, { align: 'center' });
        ...
        for (const chapter of chapters) {
          ...
          if (chapter.subChapters.length > 0) {
            for (const sub of chapter.subChapters) {
              ...
              doc.text(subPageNum, pageWidth - marginRight, currentY, { align: 'right' });
            }
          }
        }
      ...
      for (const chapter of chapters) {
        try {
          ...
          if (chapter.type === 'frontmatter') {
            ...
            doc.text(chapter.title, marginLeft, currentY);
          }
      ...
      if (lastChapter && currentY > marginTop + 40) {
