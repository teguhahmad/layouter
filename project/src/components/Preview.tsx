  ...
  const generatePdf = async () => {
    try {
      ...
      for (const chapter of frontmatterChapters) {
        doc.setFont(settings.fonts.subtitle.family);
        doc.setFontSize(settings.fonts.subtitle.size);
        const subtitleSpacing = settings.fonts.subtitle.size * settings.fonts.subtitle.lineHeight;
        doc.text(chapter.title, pageWidth / 2, currentY + subtitleSpacing, { align: 'center' });

        currentY += subtitleSpacing * 2;
        doc.setFont(settings.fonts.paragraph.family);
        doc.setFontSize(settings.fonts.paragraph.size);
        ...
        for (const paragraph of paragraphs) {
          ...
          currentY += settings.fonts.paragraph.size * settings.fonts.paragraph.lineHeight;
        }
      ...
      if (settings.tableOfContents.enabled) {
        doc.setFont(settings.fonts.subtitle.family);
        doc.setFontSize(settings.fonts.subtitle.size);
        const tocTitleSpacing = settings.fonts.subtitle.size * settings.fonts.subtitle.lineHeight;
        doc.text(settings.tableOfContents.title, pageWidth / 2, currentY + tocTitleSpacing, { align: 'center' });

        doc.setFont(settings.fonts.paragraph.family);
        doc.setFontSize(settings.fonts.paragraph.size);
        currentY += tocTitleSpacing * 2;
      ...
      for (const chapter of mainChapters) {
        ...
        for (const subChapter of chapter.subChapters) {
          ...
          currentY += 20 * settings.fonts.subtitle.lineHeight;
                  ...
                  <div key={chapter.id}>
                    ...
                    {chapter.subChapters.map((sub, subIndex) => (
                      <div key={sub.id} className="flex items-center ml-8 mt-1">
                        <span className="flex-1 font-bold">{sub.title}</span>
                        <span className="ml-2">
                          {chapter.type === 'frontmatter'
